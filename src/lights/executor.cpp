...
CommandExecutor::CommandExecutor() : m_pBytecodeStore(0), m_currentColor(),
                                     m_currentPyroChannels(0), m_ended(true),
                                     m_clockSkewCompensationFactor(1), m_resetClockFlag(false),
...
void CommandExecutor::checkAndFireTriggers(unsigned long now)
{
  ...
  for (i = 0; i < n; i++)
  {
    fired = m_triggers[i].checkAndFireWhenNeeded(now);
    if (fired)
    {
...
void CommandExecutor::delayExecutionUntil(unsigned long ms)
{
  ...
  Console.print(F(" Waiting until: "));
  Console.print(ms);
  Console.print(F(" msec (internal), "));
  Console.print(actualCompensatedDelay);
  Console.println(F(" msec (absolute)"));
  */
...
void CommandExecutor::executeActionOfTrigger(const Trigger *trigger)
{
  ...
  case TriggerActionType::JUMP_TO_ADDRESS:
    /*
      Console.print(" Triggered jump to address ");
      Console.println(action.arguments.address);
      */
...
void CommandExecutor::executeNextCommand()
{
  ...
  if (m_pBytecodeStore && !m_pBytecodeStore->suspended()) {
    Console.print(F(" [now="));
    Console.print(absoluteToInternalTime(m_currentCommandStartTime));
    Console.print(F(" ms, cum="));
    Console.print(m_cumulativeDurationSinceStart);
    Console.print(F(" ms] Command: "));
    Console.println(commandCode);
  }
  */
...
void CommandExecutor::fadeColorOfLEDStrip(sb_rgb_color_t color)
{
  ...
  /*
  Console.print(F(" Started transition with compensated duration = "));
  Console.println(actualDuration);
  */
}
...
unsigned long CommandExecutor::handleDelayByte()
{
  ...
  /*
  Console.print(F(" Delay: "));
  Console.print(duration);
  Console.println(F(" msec"));
  */
...
EasingMode CommandExecutor::handleEasingModeByte()
{
  ...
  /*
  Console.print(F(" Easing mode: "));
  Console.println(easingModeByte);
  */
...
void CommandExecutor::handleFadeToColorFromChannelsCommand()
{
  ...
  /*
  Console.print(F(" Arguments: "));
  Console.print(channelIndices[0]);
  Console.print(' ');
  Console.print(channelIndices[1]);
  Console.print(' ');
  Console.println(channelIndices[2]);
  */
...
void CommandExecutor::handleLoopBeginCommand()
{
  ...
  /*
  Console.print(F(" Starting loop #"));
  Console.print(m_loopStack.size());
  Console.print(F(" with "));
  Console.print(iterations);
  Console.println(F(" iteration(s)"));
  */
...
void CommandExecutor::handleLoopEndCommand()
{
  ...
  if (jumpTo == BYTECODE_LOCATION_NOWHERE) {
    ...
    Console.print(m_loopStack.size());
    Console.println(F(" terminated"));
  } else {
    Console.print(F(" Restarting loop #"));
    Console.println(m_loopStack.size());
  }
  */

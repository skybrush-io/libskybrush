...
public:
   ...
   * Constructor.
   */
  explicit CommandExecutorTransitionHandler(CommandExecutor *pExecutor = 0) : m_pExecutor(pExecutor), startColor(), endColor() {}
...
public:
   ...
   *        that the clock of the Arduino is at the given timestamp.
   *
   * \param arduinoClock  the value of the clock of the Arduino. Typically,
   *        you need to pass the value of \c millis() as the argument.
   *        However, you may pass in any arbitrary value to allow the
   *        simulation of an Arduino environment.
   ...
   *         was later.
   */
  unsigned long clock(unsigned long arduinoClock) const
  {
    return arduinoClock - m_lastClockResetTime;
  }
   ...
   * we can "simulate" the clock of the Arduino from the outside if needed.
   */
  void resetClock()
  {
   ...
   *
   * See also the \c CONFIG_CLOCK_SKEW_COMPENSATION macro in \c config.h
   */
   ...
   *
   * \param now  the current timestamp of the Arduino clock. The executor
...
private:
   ...
   *                    of the clock of the Arduino when the function is
   *                    called.
   */
